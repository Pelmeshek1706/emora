...
class FaceData:
    ...
    def to_dict(self, keep_face_image: bool = False) -> Dict[str, Any]:
        """
        ---------------------------------------------------------------------------------------------------
    ...
    def set_face_from_image(self, image: np.ndarray) -> None:
        """
        ---------------------------------------------------------------------------------------------------
...
def deepface_dict_to_facedata(data_dict: Dict[str, Any], frame_idx: int) -> FaceData:
    """
    ---------------------------------------------------------------------------------------------------
...
def extract_faces(
    frame: np.ndarray,
    frame_idx: int,
    rgb_back_ends: List[str],
    back_end_str: str = 'mtcnn',
    align: bool = True,
...
def prep_face_data_for_embed(
    facedata_list: List[FaceData],
    frame: np.ndarray,
) -> List[FaceData]:
    """
    ---------------------------------------------------------------------------------------------------
...
def embed_faces(face_data_list: List[FaceData], model_name: str = 'Facenet') -> List[FaceData]:
    """
    ---------------------------------------------------------------------------------------------------
...
def load_facedata_from_video(
    ...
    detector_backend: str = 'mtcnn',
    model_name: str = 'Facenet',
    n_frames: float = np.inf,
    capture_n_per_sec: int = 3
) -> Tuple[List[FaceData], float, int]:
    """
    ---------------------------------------------------------------------------------------------------
...
def facedata_list_to_df(facedata_list: List[FaceData]) -> pd.DataFrame:
    """
    ---------------------------------------------------------------------------------------------------
...
def cluster_facedata(
    face_data_across_frames: List[FaceData],
    threshold: float,
    n_clusters: int = 2
) -> pd.DataFrame:
    """
...
def prep_face_clusters_for_output(
    facedata_df: pd.DataFrame,
    min_frames_face_present: int,
    capture_n_frames_per_second: int,
    fps: float,
    num_frames_vid: int,
    n_clusters: int,
    bbox_cols: List[str],
...
def preprocess_face_video(
    video_path: str,
    n_people: int = 2,
    capture_n_frames_per_second: int = 3,
    model_name: str = 'Facenet',
    detector_backend: str = 'mtcnn',
    face_threshold: float = .95,
    min_sec_face_present: int = 3,
    n_frames: float = np.inf,
    interpolate: bool = True
) -> Tuple[Dict[int, List[BBoxDict]], pd.DataFrame]:
    """
